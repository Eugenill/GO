...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	//ctx = context.WithValue(ctx, key(50), int64(100)) //-> here we specify that we want every requestId in the context to be 100
	...
	case <-ctx.Done():
		err := ctx.Err()
		Println(ctx, err.Error())                                  //will print de log "context canceled"
